 ...
 *
 */
class Data_Battle_Weekly extends Data_Abstract
{
	/**
	 * 单例对象
	 * @var	User_Model[]
	...
	public function __construct( $userId , $lock = false  )
	{
		...
		parent::__construct( $userId , 'battle_weekly' , $lock  );
	...
	public function setData( $addData )
	{
		...
		$this->updateToDb( 'battle_weekly' , self::DATA_ACTION_ADD , $this->data['data'][$maxKey+1] );
	}
	...
	public function setPassed( $floor , $room )
		...
		foreach( $this->data['data'] as $id => $data )
		{
		   if( $floor == $data['floor'] && $room == $data['room'] && $data['pass'] == 0 )
		   {
			 ...
			 $this->updateToDb( 'battle_weekly' , self::DATA_ACTION_UPDATE , $this->data['data'][$id] );
		   }
