...
class PHPExcel
{
	...
	*/
    ...
    public function exportArray( $exportData )
    {
        $docName = $exportData[0];
        $firstfileds = $exportData[1];
        $datafileds =  $exportData[2];
        $data =  $exportData[3];
        
        if( count( $firstfileds) != count( $datafileds))
        {
            return false;
        }
        ...
        // Add some data
        $A = 65;
        foreach ( $firstfileds as  $key => $val  )
        {
            $index = chr( $A ).'1';
            $this->setActiveSheetIndex(0)->setCellValue( $index, $val );

            $this->getActiveSheet()->getStyle(  $index  )->getAlignment()->setHorizontal(  PHPExcel_Style_Alignment::HORIZONTAL_RIGHT );
            $this->getActiveSheet()->getStyle(  $index  )->getAlignment()->setVertical(  PHPExcel_Style_Alignment::VERTICAL_CENTER );
            $A++;
        }
        ...
        {
            $A = 65;
            foreach ( $datafileds as  $filed )
            {
                $index = chr( $A ).$row;
 
                $this->setActiveSheetIndex(0)->setCellValue( $index ,  $line[$filed] );
                $this->getActiveSheet()->getStyle(  $index  )->getAlignment()->setHorizontal(  PHPExcel_Style_Alignment::HORIZONTAL_RIGHT );
                $this->getActiveSheet()->getStyle(  $index  )->getAlignment()->setVertical(  PHPExcel_Style_Alignment::VERTICAL_CENTER );
                $A++;
        ...
        // Rename worksheet
        $this->getActiveSheet()->setTitle( $docName );
